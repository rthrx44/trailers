...
export const MovieBackdrop = ({movies}) => {
        ...
        {movies.map((movie, idx) => (
            <SwiperSlide key={idx}>
              ...
              <div className='absolute top-0 w-full h-full text-center flex items-end px-4 bg-gradient-to-t from-[#141414] from-10%'>
                <div className='flex flex-col justify-center pr-4 mb-16 xs:mb-40'>
        ...
        {moviesBackdrop.map((movie, idx) => (
            <SwiperSlide key={idx}>
              ...
              <div className='absolute top-0 w-full h-full grid grid-cols-2 bg-gradient-to-t from-[#141414] from-10%'>
                ...
                <div className='order-2 flex flex-col justify-center pr-6 lg:pr-10 xl:pr-20 select-none'>
                  <h1 className='text-white text-3xl font-extrabold mb-2 lg:text-4xl xl:text-5xl'>{movie.title}</h1>
                  <p className='text-white text-xs tracking-widest lg:text-sm xl:text-base'>{movie.overview}</p>
                </div>
              </div>
            </SwiperSlide>
          ))}
